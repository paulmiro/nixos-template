...
let
  cfg = config.my-config.home-manager;
...
in
{
  ...
  options.my-config.home-manager = {
    enable = lib.mkEnableOption "enable home-manager";

    profile = lib.mkOption {
      description = "which home-mamager profile to use for your user";
      type = profileType;
      default = "common";
    };
  ...
  config = lib.mkIf cfg.enable {
    ## home-manager allows you to define packages and program configuration for each user
